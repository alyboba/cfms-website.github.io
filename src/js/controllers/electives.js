...
export default class ElectivesController extends FirebaseConnection {
	...
	handleListChange(id, array, list, name) {
		...
		if (array.length > 0) {
			if(array[0] == "Empty"){
				this.utils.displayVexAlert("There are currently no electives available for this school! Please contact the <a href='mailto:it@cfms.org'>National It Officer</a> to have an elective added!");
				$('#schools-list').val('notSelected'); //Resetting option back on user...
			}
			else {
